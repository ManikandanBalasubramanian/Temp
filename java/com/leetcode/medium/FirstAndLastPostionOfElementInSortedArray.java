...
pubic class FirstAndLastPositionOfElementInSortedArray {
	    public int[] searchRange(int[] nums, int target) {
        int [] indices = new int[] {-1,-1};
        int len = nums.length;
        
        if(nums == null || nums.length <= 0 || target < nums[0] || target > nums[len-1]) return indices;
        
        if(nums[0] == target && nums[len -1] == target) return new int[] {0, len-1};
       
        
        //Binary Search 1st Index
